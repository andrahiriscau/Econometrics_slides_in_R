...
- This is a key skill for anyone doing anything with data - even if you never run a regular OLS linear regression again, pretty much everything else in applied stats builds off of it in some way
- Another thing we've been doing is thinking about how to design and add controls to that regression to *identify* our effect of interest
...
- Controlling for `County`, we automatically **control for all variables that only have between variation**, whatever they are, even if we can't measure them directly or didn't think about them
