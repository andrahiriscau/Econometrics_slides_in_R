...
build_pdf("Review_2.html")

```

# Outline

Read: [Chapter 4- Describing Relationships](https://theeffectbook.net/ch-DescribingRelationships.html)
...
- If $E(CoffeeCupsPerDay |$ $Occupation = Professor,$ $Gender = Female) = 2.13$, then 2.13 of my cups are "explained by" occupation and gender, and $3 - 2.13 = .83$ of my cups are "not explained by" those two things
...
- This doesn't necessarily mean that 2.13 of my cups are **because** I'm a professor and a female, but rather that 2.13 of my cups are **what would be expected** given what you know about me
...
- Following the textbook (*The Effect*), we'll be using data from Emily Oster's study of the relationship between taking Vitamin E and health outcomes 
...
- Or we can just show that information in a table ($TookVitaminE|Smoking$)

```{r, echo = TRUE}
oster %>% select(smoke,supplement_vite_single) %>% # select only those two variables
         mutate(smoke = as.factor(
           case_when(smoke == 0 ~ 'No Smoking',
                     TRUE ~ 'Smoking'))) %>% # create an indicator variable (0-not smoking, 1- smoking)
  table() %>% prop.table(margin = 1) # show the proportions
```

---
# Conditional Means

-  How about $Smoking|TookVitaminE$?
...
oster %>% select(smoke,supplement_vite_single) %>%
         mutate(smoke = as.factor(
           case_when(smoke == 0 ~ 'No Smoking',
                     TRUE ~ 'Smoking'))) %>% 
  table() %>% prop.table(margin = 2) # change the margins to 2
```
---
...
oster %>% filter(bmi < 100) %>%
  mutate(bmi_cut = cut(bmi, 8)) %>% group_by(bmi_cut) %>% #binning the data
  summarize(vite = mean(heart_health, na.rm = TRUE)) %>% # calculate the mean for non-missing obs.
...
oster %>% filter(bmi < 100) %>%
  ggplot(aes(x = bmi, y = heart_health)) + 
  geom_point(alpha = .4) +
  geom_smooth(size = 2, color = 'red', se = FALSE, method = 'lm') + # method (lm= linear model)
  scale_y_continuous(limits = c(-5,5)) +
  labs(x = 'Body Mass Index', y = 'Heart Health Index',
       caption = 'Outliers on the y-axis omitted to allow zoom-in') + 
  theme_pubr()
...
- We understand the sampling distribution of $\hat{\beta}_1$ (we will learn it in this course)
...
m1 <- feols(inspection_score ~ NumberofLocations, data = df) # run a linear model using OLS
...
- The other bonus regression gives us is the ability to add *control variables*. We will cover this in Chapter 3 (Wooldridge textbook)
...
```{r, echo = TRUE, eval = FALSE}
m2 <- feols(inspection_score ~ NumberofLocations + Year, data = df)
df <- df %>%
  mutate(inspection_score_res = resid(lm(inspection_score ~ Year)),
         NumberofLocations_res = resid(lm(NumberofLocations ~ Year)))

m3 <- feols(inspection_score_res ~ NumberofLocations_res, data = df)
