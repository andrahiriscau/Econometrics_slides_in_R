...
build_pdf("Instrumental_variables.html")
...
- We've been talking about within variation and DID, which are ways of controlling for stuff (and so solving endogeneity problems) without having to control for *everything* 
...
- What if, instead of controlling for stuff (between variation), we instead *isolated just the exogenous part of the treatment*?
...
- If we use *just the part of treatment driven by the instruments*, then *that part of treatment is exogenous* 
...
- We look for what part of $X_1$ is explained by $Z$
- But instead of *removing* that variation by controlling for $Z$...
- We *keep* that variation, and *remove all other variation in $X_1$ *
